...
export default function HqInviteAcceptForm({
  token,
  email,
  defaultFullName,
  roleName,
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    let res: Response
    try {
      res = await fetch('/api/auth/hq-invite', {
        method:  'PATCH',
  ...
  return (
    <div className="mt-6 w-full max-w-sm">
      <h1 className="text-xl font-semibold text-white">Join the Kinvox team</h1>
      <p className="mt-1 text-sm text-gray-400">Role: {roleName ?? 'Staff'}</p>
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        ...
        >
          {finalizing
            ? "You're in — taking you to Kinvox HQ…"
            : submitting
              ? 'Joining…'
              : 'Join the Kinvox team'}
        </button>
      </form>
    </div>
  )
