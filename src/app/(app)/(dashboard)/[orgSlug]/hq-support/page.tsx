...
function QueueTab({ queue, current, count, label, slug }: {
  queue:   Queue
  current: Queue
  count:   number
  label:   string
  ...
  const base = `/${slug}/hq-support`
  const href = queue === 'active' ? base : `${base}?queue=${queue}`
...
export default async function HQSupportPage({
  ...
  const queue: Queue = sp.queue === 'closed' ? 'closed' : 'active'
  ...
  const [ticketsRes, activeCountRes, closedCountRes, settingsRes, orgRes] = await Promise.all([
    ticketsQ,
    supabase
      .from('tickets')
  ...
  return (
    <div className="px-8 py-8 space-y-6">
      ...
      <div className="flex items-center gap-1 border-b border-pvx-border">
        <QueueTab queue="active" current={queue} count={activeCount} label="Active" slug={orgSlug} />
        <QueueTab queue="closed" current={queue} count={closedCount} label="Closed" slug={orgSlug} />
      </div>
