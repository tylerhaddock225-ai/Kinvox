...
import { sendHQTicketMessage } from '@/app/(app)/(admin)/hq/actions/tickets'

type MessageType = 'public' | 'internal'

const INITIAL = null as ReturnType<typeof useActionState<Awaited<ReturnType<typeof sendHQTicketMessage>>, FormData>>[0]

export default function HQReplyBox({ ticketId }: { ticketId: string }) {
  const [type, setType] = useState<MessageType>('public')
  const [state, action, isPending] = useActionState(sendHQTicketMessage, INITIAL)
    ...
    <form ref={formRef} action={action} className="space-y-3">
      ...
      {isInternal && (
        <div className="flex items-center gap-2 text-xs text-yellow-300">
          ...
          <span>Visible only to HQ staff.</span>
        </div>
      )}
