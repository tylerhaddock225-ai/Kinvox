...
import HuntingProfileForm from '@/app/(app)/(dashboard)/[orgSlug]/settings/team/HuntingProfileForm'

export type SignalSettingsState = {
  // Moved from LeadSupportState in Workstream I — these three back the
  // signal-capture controls (toggle, reply mode, credit balance) that
  // now live on the Signal Settings tab.
  ai_listening_enabled:   boolean
  signal_engagement_mode: 'ai_draft' | 'manual'
  balance:                number
  // Hunting profile fields (unchanged from prior shape).
...
// Local tokens — mirror lead-support-tab.tsx so the two tabs render
// consistently. Kept inline to avoid a third shared-tokens module.
...
export default function SignalSettingsTab({ state }: { state: SignalSettingsState }) {
  const [toggleState, toggleAction, togglePending] = useActionState(setAiListeningEnabled, null)
  const [enabled, setEnabled] = useState<boolean>(state.ai_listening_enabled)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Reply Strategy */}
      <ReplyStrategySelector current={state.signal_engagement_mode} />
      ...
      {/* Hunting Profile (the original Signal Settings tab content) */}
      <HuntingProfileForm
        orgVertical={state.orgVertical}
        initialAddress={state.initialAddress}
        initialRadius={state.initialRadius}
        initialKeywords={state.initialKeywords}
      />
